...
export default Router()
  .use(multipart())
  .post('/upload/', (req,res) => {
    let fileContentBufferPromise,fileName,contentType;
    if(req.body['file']) {
      // The file was sent directly in the form data
      fileContentBufferPromise = Promise.resolve(new Buffer(req.body['file'],'base64'));
    } else {
    ...
    const promise = fileContentBufferPromise
      ...
      .catch((err) => {console.log('error',err,err.stack); return Promise.reject(err);})
